codeunit 50105 "Google Maps API" implements "IMap Service Functions"
{
    var
    ...
    begin
        ...
        AddressUri := StrSubstNo(MapServiceFunction.Url, AddressUri) + GetAPIKeyUrlPart();

        if not SendWebRequest(AddressUri, MapServiceFunction, WebResponse) then
            exit;
    ...
    begin
        ApplicationMgt.LogTelemetry(ProcName, 'provider', 'OpenStreetMap', '', '');
    end;
    ...
    begin
        MapServiceProvider.Get(MapServiceProvider."Map Service Provider"::GoogleMapsAPI);
        exit(MapServiceProvider."API Key");
    end;

    local procedure GetAPIKeyUrlPart(): Text
    begin
        exit('&key=' + GetAPIKey());
    end;
